    ...
    private static OptionGroup makeCommonGroup(WhereIsItConfig defaults, WhereIsItConfig config) {
        return OptionGroup.createBuilder()
                ...
                .option(Option.<Integer>createBuilder()
                        ...
                        .build())
                .option(Option.<Integer>createBuilder()
                        .name(translatable("whereisit.config.common.trackingRange"))
                        .binding(
                                defaults.getCommon().trackingRangeBlocks,
                                () -> config.getCommon().trackingRangeBlocks,
                                i -> config.getCommon().trackingRangeBlocks = i
                        )
                        .description(OptionDescription.of(translatable("whereisit.config.common.trackingRange.description")))
                        .controller(opt -> IntegerSliderControllerBuilder.create(opt)
                                .range(16, 1024)
                                .step(4)
                                .formatValue(i -> translatable("whereisit.config.common.trackingRange.slider", i)))
                        .build())
                .build();
    }
}
